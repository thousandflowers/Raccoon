...
# ============================================================================
# Intervention sheet — a fill-in document for the technician
# ============================================================================
...
render_intervention_sheet() {
	...
	printf '## Issues found and resolved\n'
	...
	printf -- '---\n'
	printf '*Generated by Raccoon v%s · %s*\n' "$ver" "$date"
}
...
render_intervention_sheet_rtf() {
	...
	printf '{\\rtf1\\ansi\\ansicpg1252\\deff0{\\fonttbl{\\f0 Helvetica;}}\n'
	printf '\\fs28\n'
	printf '{\\b\\fs36 Intervention Sheet}\\par\n'
	printf '\\par\n'
	printf '{\\b Date:} %s\\par\n' "$(rtf_escape "$date")"
	printf '{\\b Technician:} %s\\par\n' "$(rtf_escape "$tech")"
	printf '{\\b Studio/Shop:} %s\\par\n' "$(rtf_escape "$shop")"
	printf '\\par\n'
	printf '{\\b\\fs32 Client}\\par\n'
	printf '%s - %s - macOS %s\\par\n' "$(rtf_escape "$client")" "$(rtf_escape "$model")" "$(rtf_escape "$os")"
	printf '\\par\n'

	printf '{\\b\\fs32 Issues found and resolved}\\par\n'
	local entry before
	for entry in ${AUDIT_RESULTS[@]+"${AUDIT_RESULTS[@]}"}; do
		_parse_entry "$entry"
		before="$(_json_check_status "${BASELINE_FILE:-}" "$_PE_NAME")"
	...
	printf '{\\b\\fs32 Hours worked}\\par\n'
	printf '%s\\par\n' "$(rtf_escape "$hours")"
	printf '\\par\n'
	printf '{\\b\\fs32 Additional notes}\\par\n'
	printf '%s\\par\n' "$(rtf_escape "$notes")"
	printf '\\par\n'
	printf '{\\i Generated by Raccoon v%s - %s}\\par\n' "$(rtf_escape "$ver")" "$(rtf_escape "$date")"
	printf '}\n'
}
...
render_fleet_md() {
	...
	printf '**Date:** %s\n\n' "${REPORT_DATE:-}"
	printf '## Summary\n\n'
	printf '%s/%s hosts reached · ✓ %s pass · ⚠ %s warn · ✗ %s fail\n\n' \
...
render_fleet_rtf() {
	...
	printf '\\par\n'
	printf '{\\b\\fs32 Summary}\\par\n'
	printf '%s/%s hosts reached - %s pass, %s warn, %s fail\\par\n' \
		"${FLEET_REACHED:-0}" "${FLEET_COUNT:-0}" \
		"${FLEET_TOTAL_PASS:-0}" "${FLEET_TOTAL_WARN:-0}" "${FLEET_TOTAL_FAIL:-0}"
	printf '\\par\n'
